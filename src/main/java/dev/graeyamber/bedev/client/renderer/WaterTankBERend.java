package dev.graeyamber.bedev.client.renderer;
...
public class WaterTankBERend implements BlockEntityRenderer<WaterTankBlockEntity> {

    private final BlockEntityRendererProvider.Context context;

    public WaterTankBERend(BlockEntityRendererProvider.Context ctx) {
        this.context = ctx;
    }

    @Override
    public void render(@NotNull WaterTankBlockEntity pBlockEntity, float pPartialTick,
    ...
    private static void drawQuad(VertexConsumer builder, PoseStack poseStack, float x0, float y0, float z0, float x1, float y1, float z1, float u0, float v0, float u1, float v1, int packedLight, int color) {
        ...
        BEDev.LOGGER.info(x0 + ' ' + y0 + ' ' + z0 + " - " + x1 + ' ' + y1 + ' ' + z1);
    }
}
